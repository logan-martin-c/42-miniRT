...
/*   Updated: 2026/03/08 23:05:21 by lomartin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_global_data *g_data)
{
	...
	if (!ft_strcmp(pair->key, "texture") && !check->tex)
		return (check->tex = 1, parse_json_string(&pair->value,
				&obj->tex_name));
	// if (!ft_strcmp(pair->key, "bump_map") && !check->bump_map)
...
// int json_cone_params(t_pair *pair, t_object *obj,
...
//     if (!ft_strcmp(pair->key, "position") && !check->pos)
//         return (check->pos = 1, parse_json_pos(&pair->value, &obj->pos,
// 		false));
//     if (!ft_strcmp(pair->key, "rotation") && !check->rot)
//         return (check->rot = 1, parse_json_pos(&pair->value, &obj->rot,
// 		true));
//     if (!ft_strcmp(pair->key, "diameter") && !check->diamet)
//         return (check->diamet = 1, parse_json_float(&pair->value,
// 		&obj->u_data.cone.diameter, _positive));
...
//     if (json.e_type != _brace)
//         return (1);
//     world->objs = ft_realloc(world->objs, world->obj_count 
//		* sizeof(t_object),
//         (world->obj_count + 1) * sizeof(t_object));
//     set_default_obj(world->objs + world->obj_count);
//     i = -1;
...
//		g_data->prog_name));
//     world->objs[world->obj_count].u_data.cone.radius = world->objs[world->obj_count].u_data.cone.diameter
//	* 0.5;
