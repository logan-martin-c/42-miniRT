...
{
	if (prev_pos.x + prev_strlen * 2 + 20 < WIN_WIDTH)
		return ((t_vect2){prev_pos.x + prev_strlen * 2 + 20, prev_pos.y});
	if (prev_pos.y < WIN_HEIGHT / 2)
		return ((t_vect2){10, WIN_HEIGHT - prev_pos.y});
	return ((t_vect2){10, WIN_HEIGHT - prev_pos.y + 20});
}
...
void	display_int(char *name, int value, t_mlx_data *mlx, bool reset)
{
	...
	total_print_len = (ft_strlen(name) + ft_strlen(int_str)) * 2 + 20;
	...
	mlx_string_put(mlx->mlx, mlx->win, pos.x + ft_strlen(name) * 2 + 20, pos.y,
		0xFFFFFF, int_str);
	pos = get_next_pos(pos, total_print_len);
}
